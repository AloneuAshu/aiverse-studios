...
function initTabNav() {
  ...
  tabs.forEach(tab => {
    tab.addEventListener('click', () => {
      ...
      // Pause trim preview player
      const trimPlayer = document.getElementById('trimPreviewPlayer');
      if (trimPlayer && target !== 'trimStudio') {
        trimPlayer.pause();
        const playBtn = document.getElementById('trimBtnPlayPause');
      ...
      } else if (target === 'trimStudio') {
        panel = document.getElementById('panelTrimStudio');
      }
...
function initSubtitles() {
  ...
  // ── GENDER DETECTION: wire up toggle & update transcribe request ──────────
  const subDetectGender = $('subDetectGender');

  // Override Step 1 TRANSCRIBE button to include gender flag
  // (We re-attach by wrapping the existing listener via a flag approach)
  subDetectGender && subDetectGender.addEventListener('change', () => {
    subBtnGenerate.textContent = subDetectGender.checked
      ? 'STEP 1 - TRANSCRIBE + DETECT GENDER'
      : 'STEP 1 - TRANSCRIBE AUDIO TO SRT';
  });

  // Patch the generate button to pass detectGender
  subBtnGenerate.removeEventListener('click', subBtnGenerate._genderPatchedHandler);
  subBtnGenerate._genderPatchedHandler = async () => {
    const fp = subFilePath.value.trim();
    if (!fp) return;
    ...
    subTranslateSection.classList.add('disabled-state');
    subBurnSection.classList.add('disabled-state');
    $('subTtsSection') && $('subTtsSection').classList.add('disabled-state');
    subProgressSection.classList.remove('hidden');
    subProgressFill.style.width = '0%';
    subStatusText.textContent = detectGender
      ? 'Extracting audio, initializing transcription + gender detection...'
      : 'Extracting audio and initializing transcription...';
    try {
      const r = await fetch('/api/subtitle-generate', {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ filePath: fp, langCode: subLanguageSelect.value, detectGender })
      });
      const d = await r.json();
      if (!r.ok) throw new Error(d.error || 'Generation failed');
      ...
      const poll = setInterval(async () => {
        try {
          ...
          if (sd.status === 'transcribing') {
            ...
            // Load SRT preview with gender badges
            try {
              const txt = await (await fetch(sd.srtPath)).text();
              if (detectGender) {
                subSrtPreviewText.value = txt;
                // Build gender summary card
                if (sd.genderSummary) {
            ...
            subTranslateSection.classList.remove('disabled-state');
            subBurnSection.classList.remove('disabled-state');
            $('subTtsSection') && $('subTtsSection').classList.remove('disabled-state');
  ...
  // Remove old listener and attach patched one
  const oldBtn = subBtnGenerate.cloneNode(true);
  subBtnGenerate.parentNode.replaceChild(oldBtn, subBtnGenerate);
  // Re-reference after DOM replacement
  const newSubBtnGenerate = $('subBtnGenerate');
  newSubBtnGenerate.addEventListener('click', newSubBtnGenerate._genderPatchedHandler = async () => {
    const fp = subFilePath.value.trim();
    if (!fp) return;
    const detectGender = $('subDetectGender') ? $('subDetectGender').checked : false;
  ...
  // ── TTS ENGINE + SOURCE SELECTOR ─────────────────────────────────────────
  let selectedTtsEngine = 'online';
  let selectedTtsSrc    = 'original'; // 'original' | 'translated'
  let currentTranslatedSrtPath = null; // set when translation completes
  let currentTranslatedLang    = 'en'; // target lang short code
  ...
  // Source toggle (Original / Translated)
  ...
  if (subBtnTtsGenerate) {
    subBtnTtsGenerate.addEventListener('click', async () => {
      ...
      // Language: if translated, use the target lang; else derive from source lang selector
      let lang;
      if (useTranslated) {
  ...
  // ── Translated SRT quick "Generate Audio" button ──────────────────────────
  ...
  // ── Hook into translate complete to store translated SRT path + lang ───────
  // We patch currentTranslatedSrtPath via the existing translate poll
  // by overriding the translate button handler to also set our vars.
  const origTranslateBtn = $('subBtnTranslate');
  if (origTranslateBtn) {
    origTranslateBtn.addEventListener('click', () => {
      // After translation completes, the existing handler sets currentBurnSrtPath.
      // We observe subTranslatedLangLabel changes as a proxy.
      const obs = new MutationObserver(() => {
        ...
        if (tSrtEl && tSrtEl.href && !tSrtEl.href.endsWith('#')) {
          ...
          // Derive lang from target selector
          const sel = $('subTargetLangSelect');
          currentTranslatedLang = sel ? sel.value.split('-')[0] : 'en';
          // Also flip the source selector to "Translated" for convenience
          if (ttsSrcTranslated) {
            [ttsSrcOriginal, ttsSrcTranslated].forEach(b => b && b.classList.remove('active'));
...
function initTrimStudio() {
  ...
  // Update UI overlay (handles, selection box, tooltips, times)
  ...
  // Playhead sync with video player timeupdate
  trimPreviewPlayer.addEventListener('timeupdate', () => {
    updateTrimPlayhead();
    // Loop if it exceeds the Out point
  ...
  function buildTrimFilmstrip(filePath, duration) {
    ...
    const thumbW = 75; // Width of each filmstrip frame
    const numFrames = Math.max(8, Math.floor(trackW / thumbW));
    
    // Set width of zoom wrapper in pixels to expand scroll width
  ...
  // Keyboard Shortcuts (when this tab is active)
